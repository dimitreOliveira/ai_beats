...
from common import AUDIO_CLIP_DIR, MUSIC_FINAL_DIR, VIDEO_DIR, timer


def generate_audio_clip(track_video_prompt_paths):
    for track_path, video_prompt_path in track_video_prompt_paths:
        ...
        video_paths = sorted(
            [p for p in video_prompt_path.glob("*.mp4") if p.is_file()]
        )
...
track_video_prompt_paths = zip(track_paths, video_prompt_paths[: len(track_paths)])

with timer("Audio clip creation"):
    generate_audio_clip(track_video_prompt_paths)
